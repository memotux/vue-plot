import { camelize, capitalize, hyphenate, is } from "../utils"

// @ts-ignore
const __DEV__ = import.meta.dev

type Style = string | Record<string, string | string[]> | null

// const displayRE = /(^|;)\s*display\s*:/

const CSS_VAR_TEXT: unique symbol = Symbol(__DEV__ ? 'CSS_VAR_TEXT' : '')

export function patchStyle(el: Element, prev: Style, next: Style): void {
  const style = (el as HTMLElement).style
  const isCssString = is.str(next)
